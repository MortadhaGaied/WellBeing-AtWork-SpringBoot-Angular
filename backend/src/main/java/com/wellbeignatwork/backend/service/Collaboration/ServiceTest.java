...
import com.wellbeignatwork.backend.repository.Collaboration.ISearchRepo;
                    ...
                    if(iResultRepo.getScore(u.getId()) >= 1000 && iResultRepo.getScore(u.getId()) <=1500 && iResultRepo.getNbrQuiz(u.getId()) == 15 )
                    {
                        ...
                        for (Result r : iResultRepo.getResultByIdUserAndAndIdTest(u.getId(),f.getIdTest()))
                        {
                            ...
                            if (status && !fin)
                            {
                                ...
                                this.emailSenderService.sendSimpleEmail(u.getEmail()," Congratulations Mr's : "+u.getDisplayName()+" you have finished your Courses  " ," Certification At : "+ new Date()+"  in Courses of Domain "+f.getDomain()+" "+" And Niveau : " +f.getLevel() +" .");
                                fin=true; /// return /////
                            }
                ...
                if (iTestRepo.getNbrEmployeeByTestId(idTest) < test.getNbrParticipant() && employee.getProfession() == Profession.EMPLOYEE) {
                    ...
                    if (iTestRepo.getNbrEmployeeByTest(idTest, test.getDomain()) < 2 || employee.getId().equals(user.getId())) {
                        if (iTestRepo.getNbrEmployeeByTest(Math.toIntExact(idUser), test.getDomain()) < 3) {
                            ...
                            this.emailSenderService.sendSimpleEmail(employee.getEmail(), "we don't have acces to add two coursus in same domain ", "we have 2 (MAX formation in this domain) NAME : " + employee.getDisplayName() + " .");
                        }
                    ...
                    this.emailSenderService.sendSimpleEmail(employee.getEmail(), "Learner list complete  ", " We have in this courses " + test.getNbrParticipant() + " number of learner Maximum" + employee.getDisplayName() + " .");
